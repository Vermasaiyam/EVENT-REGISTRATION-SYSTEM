...
import { generateClubCreationEmailHtml, generateClubEditEmailHtml, generatePasswordResetEmailHtml, generateResetSuccessEmailHtml, generateWelcomeEmailHtml, htmlContent } from "../mailtrap/htmlEmail";
...
export const sendClubEditEmail = async (options:any) => {
    let config = {
        service: "gmail",
        auth: {
            user: process.env.NODEMAILER_USER,
    ...
    const mailOptions = {
        ...
        subject: options.subject,
        text: options.message,
        html: generateClubEditEmailHtml(options.clubName, options.eventTypes, options.coreTeam, options.clubEmail, options.instaHandle, options.linkedinHandle, options.xHandle),
    };
